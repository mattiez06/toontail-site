            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <div className="mt-4 flex gap-2">
                {p.status === "in_stock" ? (
                  <button
                    onClick={() => onAdd(p)}
                    className="flex-1 px-4 py-2 rounded-xl bg-sky-600 text-white font-medium hover:bg-sky-700"
                  >
                    Add to cart
                  </button>
                ) : (
        ...
        <div className="p-5 space-y-4 overflow-auto h-[calc(100%-200px)]">
          ...
          {items.map(({ line, product }) => (
            <div key={product.id} className="flex gap-3 items-center">
              {product.img && (
                <img src={product.img} alt={product.name} className="w-20 h-20 object-cover rounded-lg border" />
              )}
